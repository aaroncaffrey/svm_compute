...
namespace svm_compute
{
    public static class libsvm_grid
    {
        public class best_rate_container
        {
            ...
            public cross_validation.libsvm_cv_perf best_libsvm_cv_perf;
            //public double? best_log2c = 0;

            //public double? best_log2g = 0;

            public void update_rate(double? cost, double? gamma, double? epsilon, double? coef0, double? degree, double? rate, cross_validation.libsvm_cv_perf libsvm_cv_perf)
            {
                ...
                if (print_params)
                {
                    ...
                    if (program.write_console_log) program.WriteLine($@"{nameof(update_rate)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
                }
                ...
                lock (_rate_lock)
                {
                    if (
                        (best_rate == null) || 
                        (rate != null && rate.Value > best_rate.Value) ||
                        
                        ((rate != null && best_rate != null && rate.Value == best_rate.Value) && (
                         (cost != null && best_cost != null && Math.Abs(cost.Value) < Math.Abs(best_cost.Value))) ||
                         (gamma != null && best_gamma != null && Math.Abs(gamma.Value) < Math.Abs(best_gamma.Value)) ||
                         (epsilon != null && best_epsilon != null && Math.Abs(epsilon.Value) < Math.Abs(best_epsilon.Value)) ||
                         (coef0 != null && best_coef0 != null && Math.Abs(coef0.Value) < Math.Abs(best_coef0.Value)) ||
                         (degree != null && best_degree != null && Math.Abs(degree.Value) < Math.Abs(best_degree.Value)))
                       )
                    {
                        best_rate = rate;
                        best_cost = cost;
                        ...
                        best_libsvm_cv_perf = libsvm_cv_perf;
                    }
                }
            }
        }
            ...
            )
        {
            ...
            if (print_params)
            {
                ...
                if (program.write_console_log) program.WriteLine($@"{nameof(cross_validate_model_internal)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            }


            var max_tasks_internal_cv = (int?)null;

            var perf = libsvm_caller.cross_validate_model_internal_logic(inner_cv_svm_implementation, full_training_file, train_stdout_file, train_stderr_file, output_threshold_adjustment_performance, model_index, cost, gamma, epsilon, coef0, degree, weights, svm_type, kernel, inner_cv_folds, probability_estimates, shrinking_heuristics, point_max_time, process_max_time, max_tasks_internal_cv, echo, echo_err);
            ...
            )
        {
            ...
            if (print_params)
            {
                ...
                if (program.write_console_log) program.WriteLine($@"{nameof(cross_validate_model_libsvm)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            }
            ...
            var train_result = libsvm_caller.train(inner_cv_svm_implementation, training_file, model_filename, train_stdout_file, train_stderr_file, cost, gamma, epsilon, coef0, degree, weights, svm_type, kernel, eval_method, inner_cv_folds, probability_estimates, shrinking_heuristics, point_max_time, echo, echo_err);
        ...
        //}
        public static bool libsvm_eval_method_supported(List<int> cross_validation_metric_class_list = null, performance_measure.confusion_matrix.cross_validation_metrics cross_validation_metrics = performance_measure.confusion_matrix.cross_validation_metrics.ACC)
        {
            ...
            )
        {
            ...
            if (svm_implementation == libsvm_caller.svm_implementation.libsvm_eval)
            {
                use_internal_cv = !libsvm_eval_method_supported(cross_validation_metric_class_list, cross_validation_metrics);
            }
            ...
            //if (program.write_console_log) program.WriteLine($"use_internal_cv = {use_internal_cv}");
            ...
            for (var index = 0; index < search_grid_points.Count; index++)
            {
                ...
                if (max_tasks == 0)
                {
                    grid_search_task_logic(svm_implementation, training_file, train_stdout_file, train_stderr_file, output_threshold_adjustment_performance, model_index, br, use_internal_cv, cross_validation_metric_class_list, cross_validation_metrics, svm_type, kernel, inner_cv_folds, probability_estimates, shrinking_heuristics, point.cost, point.gamma, point.epsilon, point.coef0, point.degree, weights, point_max_time, process_max_time, echo, echo_err);
                }
                else
                {
                    var task = Task.Run(() => grid_search_task_logic(svm_implementation, training_file, train_stdout_file, train_stderr_file, output_threshold_adjustment_performance, model_index, br, use_internal_cv, cross_validation_metric_class_list, cross_validation_metrics, svm_type, kernel, inner_cv_folds, probability_estimates, shrinking_heuristics, point.cost, point.gamma, point.epsilon, point.coef0, point.degree, weights, point_max_time, process_max_time, echo, echo_err));
                    tasks.Add(task);
                    ...
                    if (index != search_grid_points.Count - 1)
                    {
                        ...
                        {
                            if (program.write_console_log) program.WriteLine($@"grid_parameter_search(): Task.WaitAny(tasks.ToArray<Task>());", true, ConsoleColor.Cyan);
                            ...
                            {
                                program.WriteLineException(e, nameof(grid_parameter_search));
                            }
            ...
            {
                if (program.write_console_log) program.WriteLine($@"grid_parameter_search(): Task.WaitAll(tasks.ToArray<Task>());", true, ConsoleColor.Cyan);
                try
                {
            ...
            )
        {
            ...
            if (print_params)
            {
                ...
                if (program.write_console_log) program.WriteLine($@"{nameof(grid_search_task_logic)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            }

            var cv_rate = 0d;

            cross_validation.libsvm_cv_perf libsvm_cv_perf = null;

            var eval_method = get_modded_libsvm_eval_method(cross_validation_metric_class_list, cross_validation_metrics);

            if (use_internal_cv)
            {
                //if (program.write_console_log) program.WriteLine($@"Warning: using internal CV (slow)");
                throw new Exception($@"Warning: using internal CV (slow)");

                libsvm_cv_perf = null;
                var cv = cross_validate_model_internal(inner_cv_svm_implementation, training_file, train_stdout_file, train_stderr_file, output_threshold_adjustment_performance, model_index, cross_validation_metric_class_list, cross_validation_metrics, svm_type, kernel, inner_cv_folds, probability_estimates, shrinking_heuristics, cost, gamma, epsilon, coef0, degree, weights, point_max_time, process_max_time, echo, echo_err);
                cv_rate = cv ?? 0d;
            }
            else
            {
                libsvm_cv_perf = cross_validate_model_libsvm(inner_cv_svm_implementation, training_file,
                    train_stdout_file, train_stderr_file,

                model_index, svm_type, kernel, eval_method, inner_cv_folds, probability_estimates, shrinking_heuristics, cost, gamma, epsilon, coef0, degree, weights, point_max_time, process_max_time, echo, echo_err);
                cv_rate = libsvm_cv_perf?.v_cross_validation ?? 0d;
            }

            if (cv_rate > 0) // != null)
            {
                best_rate.update_rate(cost, gamma, epsilon, coef0, degree, cv_rate, libsvm_cv_perf);//, cv_rate.g, cv_rate.rate.Value);
            }
        }
    }
}
