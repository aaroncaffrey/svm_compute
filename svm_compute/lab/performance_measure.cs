...
{
    // todo: add interpolation to the AUC-PR calculations.
    ...
    public static class performance_measure
    {
        ...
        public class confusion_matrix
        {
            ...
            public void calculate_metrics(bool calculate_auc, List<prediction> prediction_list)
            {
                ...
                if (calculate_auc && prediction_list != null && prediction_list.Count > 0)
                {
                    ...
                    roc_xy_str_all = "FPR;TPR/" + String.Join("/", p_roc_xy_all.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());
                    roc_xy_str_11p = "FPR;TPR/" + String.Join("/", p_roc_xy_11p.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());

                    pr_xy_str_all = "TPR;PPV/" + String.Join("/", p_pr_xy_all.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());
                    pr_xy_str_11p = "TPR;PPV/" + String.Join("/", p_pr_xy_11p.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());

                    pri_xy_str_all = "TPR;PPV/" + String.Join("/", p_pri_xy_all.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());
                    pri_xy_str_11p = "TPR;PPV/" + String.Join("/", p_pri_xy_11p.Select(a => $"{Math.Round(a.x, 6)};{Math.Round(a.y, 6)}").ToList());
            ...
            public static confusion_matrix Average2(List<confusion_matrix> cm)
            {
                ...
                var result = new confusion_matrix()
                {
                    ...
                    cv_rate = cm.Select(a => a.cv_rate).Average(),
            ...
            public static List<confusion_matrix> Average1(List<confusion_matrix> confusion_matrices)//, bool class_specific = true)
            {
                ...
                {
                    (nameof(confusion_matrices),confusion_matrices.ToString()),

                };


                program.WriteLine($@"{nameof(Average1)}({String.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
                //#endif
            ...
            )
        {
            ...
            //program.WriteLine($@"{nameof(count_prediction_error)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            //#endif
            ...
            for (var i = 0; i < actual_class_id_list.Count; i++)
            {
                ...
                var confusion_matrix1 = new confusion_matrix()
                {
                    ...
                    thresholds = prediction_list.Select(a => a.probability_estimates.First(b => b.class_id == actual_class_id).probability_estimate).Distinct().OrderByDescending(a => a).ToList(),
        ...
        public static double area_under_curve_trapz(List<(double x, double y)> coordinate_list)//, bool interpolation = true)
        {
            ...
            //program.WriteLine($@"{nameof(area_under_curve_trapz)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            //#endif
        ...
        public static List<prediction> load_prediction_file_regression_values(string test_file, string prediction_file)
        {
            //#if DEBUG
            //var param_list = new List<(string key, string value)>()
            //{
            ...
            //program.WriteLine($@"{nameof(load_prediction_file_regression_values)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            //#endif

            if (!File.Exists(test_file))
            {
                throw new Exception("Error: test file missing");
            }

            if (!File.Exists(prediction_file))
            {
                throw new Exception("Error: prediction file missing");
            }

            var test_file_lines = File.ReadAllLines(test_file);
            ...
            var test_file_data = test_file_lines.Where(a => !string.IsNullOrWhiteSpace(a) && int.TryParse(a.Split().First(), out var line_cid)).Select(a => a.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)).ToList();
            //test_file_data = test_file_data.Where(a => a.Length > 1).ToList();
            if (test_file_lines.Length == 0) return null;


            var prediction_file_lines = File.ReadAllLines(prediction_file);
            var prediction_file_data = prediction_file_lines.Where(a => !string.IsNullOrWhiteSpace(a) && int.TryParse(a.Split().First(), out var line_cid)).Select(a => a.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)).ToList();
            if (prediction_file_lines.Length == 0) return null;

            var probability_estimate_class_labels = new List<int>();
            if (prediction_file_lines.First().Split().First() == "labels")
            {
                probability_estimate_class_labels = prediction_file_lines.First().Split().Skip(1).Select(int.Parse).ToList();
            }
            ...
            for (var prediction_index = 0; prediction_index < test_file_data.Count; prediction_index++)
            {
                ...
                {
                    comment = test_file_comments[prediction_index],
        ...
        public static List<confusion_matrix> load_prediction_file(string test_file, string prediction_file, bool output_threshold_adjustment_performance)
        {
            //#if DEBUG
            //var param_list = new List<(string key, string value)>()
            //{
            ...
            //program.WriteLine($@"{nameof(load_prediction_file)}({string.Join(", ", param_list.Select(a => $"{a.key}=\"{a.value}\"").ToList())});");
            //#endif

            var prediction_list = load_prediction_file_regression_values(test_file, prediction_file);
            ...
            Calculate_ROC_PR_AUC(List<prediction> prediction_list, int positive_id, threshold_type threshold_type = threshold_type.all_thresholds)
        {
            ...
            {
                thresholds = prediction_list.Select(a => a.probability_estimates.First(b => b.class_id == positive_id).probability_estimate).Distinct().OrderByDescending(a => a).ToList();
            }
            ...
            var threshold_prediction_list = thresholds.Select(t => (positive_threshold: t, prediction_list: prediction_list.Select(pl => new prediction()
            {
                ...
                default_predicted_class = pl.probability_estimates.First(e => e.class_id == positive_id).probability_estimate >= t ? positive_id : negative_id,
