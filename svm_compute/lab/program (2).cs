    ...
    public class program
    {
        ...
        public static void load_hosts()
        {
            ...
            try
            {
                if (File.Exists(hosts_file) && new FileInfo(hosts_file).Length > 0)
                {
                    lock (cross_validation_remote.compute_unit_hostnames_lock)
                    {
                        ...
                        var hosts_data = hosts_lines.Select(a =>
                        {
                            ...
                            return (hostname: x[0], port: int.Parse(x[1]), enabled: string.Equals(x[2], "true", StringComparison.InvariantCultureIgnoreCase) || x[2] == "1");
                        }).ToList();
                ...
                {
                    program.WriteLine("Hosts file is missing.", true, ConsoleColor.Red);
                }
            }
            catch (Exception e)
            {
        ...
        public static void master_compute(CancellationToken token)
        {
            ...
            /////// start !!! temp code for a specific task
            //var done_file = $@"C:\Users\aaron\Desktop\aaindex - all - best ppf overall - f1 - linear\22-04-2019_04-07-20_all_cm_average.csv";
            //var done_data = File.ReadAllLines(done_file).Skip(1).ToList();
            //done_data = done_data.Select(a => a.Substring(0, a.IndexOf(','))).ToList();
            //var done_data_split = done_data.Select(a =>
            //{
            //    var b = a.Split(';').ToList();

            //    return (category: b[0], dimension: b[1], source: b[2], alphabet: b[3], group: b[4], set_id: b[5]);
            //}).ToList();

            //dataset_headers = dataset_headers.Where(a => !done_data_split.Any(b => a.category == b.category && a.alphabet == b.alphabet && a.group == b.group && a.source == b.source && a.dimension == b.dimension)).ToList();
            /////// end !!! temp code for a specific task
            ...
            var max_features_per_group = 600;
            Console.WriteLine($"Removing feature groups with more than {max_features_per_group} features");
            features_input = features_input.Where(a => a.set_members.Count < max_features_per_group).ToList();


            Console.WriteLine($"Ordering feature groups by size");
            ...
            Console.WriteLine($"Setting feature_selection_unidirectional() parameters.");
            ...
            Console.WriteLine($"Calling run_unidirectional_feature_selection()");
