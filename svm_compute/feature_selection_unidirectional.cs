...
namespace svm_compute
{
    public class feature_selection_unidirectional
    {
        ...
        public /*static*/ void save_task(iterative_task result)
        {
            ...
            if (save_task_data)
            {
                ...
                var iterative_task_result_str = result.ToString();
                ...
                if (save_all_cm)
                {
                    ...
                    var all_cm_data = result.all_cm.Select(cm => $"{iterative_task_result_str},{cm.ToString()}").ToList();
                    ...
                    lock (save_task_lock)
                    {
                        if (!File.Exists(all_cm_filename) || new FileInfo(all_cm_filename).Length == 0)
                        {
                            ...
                            program.AppendAllLines(all_cm_filename, new string[] { h });

                        }

                        program.AppendAllLines(all_cm_filename, all_cm_data);
                ...
                if (save_all_cm_average)
                {
                    ...
                    var all_cm_average_data = result.all_cm_average.Select(a => $"{iterative_task_result_str},{a.ToString()}").ToList();
                    ...
                    lock (save_task_lock)
                    {
                        if (!File.Exists(all_cm_average_filename) || new FileInfo(all_cm_average_filename).Length == 0)
                        {
                            ...
                            program.AppendAllLines(all_cm_average_filename, new string[] { h });

                        }

                        program.AppendAllLines(all_cm_average_filename, all_cm_average_data);
        ...
        public void save_direction_change()
        {
        ...
        public void save_iteration_parameters()
        {
            ...
            if (save_params)
            {
                ...
                program.WriteAllLines(Path.Combine(this.results_output_folder, $"{start_date_time}_parameters_{iteration_loop}_{iteration_recalculate}_{iteration_select}.csv"), p_lines);
            ...
            {
                var feat_header =
                    $@"iteration_loop,iteration_recalculate,iteration_select,baseline_recalculation_required,score_improved,forwards_score_improved,backwards_score_improved,go_forwards,can_go_forwards,go_backwards,can_go_backwards,direction_change,feature_selection_type,perf_selection_rule,feature_selection_performance_metrics,feature_selection_combinator,experiment_id1,experiment_id2,experiment_id3,iteration_loop,alphabet,dimension,category,source,group,members,perspectives,set_id,{performance_measure.confusion_matrix.csv_header}";

                {
                    var f_lines_cm_average = this.features_selected_average_confusion_matrices.SelectMany(a => a.cm_list
                        .Select(b =>
                            ...
                            $@"{iteration_loop},{iteration_recalculate},{iteration_select},{baseline_recalculation_required},{score_improved},{forwards_score_improved},{backwards_score_improved},{go_forwards},{can_go_forwards},{go_backwards},{can_go_backwards},{direction_change},{feature_selection_type.ToString().Replace(",", ";")},{perf_selection_rule.ToString().Replace(",", ";")},{feature_selection_performance_metrics.ToString().Replace(",", ";")},{feature_selection_combinator.ToString().Replace(",", ";")},{a.experiment_id1},{a.experiment_id2},{a.experiment_id3},{a.iteration},{a.feature_set.alphabet},{a.feature_set.dimension},{a.feature_set.category},{a.feature_set.source},{a.feature_set.@group},{string.Join("/", a.feature_set.set_members.Select(c => c.member).Distinct().OrderBy(c => c).ToList())},{string.Join("/", a.feature_set.set_members.Select(c => c.perspective).Distinct().OrderBy(c => c).ToList())},{a.feature_set.set_id},{b.ToString()}"
                        ).ToList()).ToList();
...

            {
                ...
                var f_lines_cm_all = this.features_selected_average_confusion_matrices.SelectMany(a => a.cm_all.Select(b =>
                    ...
                    $@"{iteration_loop},{iteration_recalculate},{iteration_select},{baseline_recalculation_required},{score_improved},{forwards_score_improved},{backwards_score_improved},{go_forwards},{can_go_forwards},{go_backwards},{can_go_backwards},{direction_change},{feature_selection_type.ToString().Replace(",", ";")},{perf_selection_rule.ToString().Replace(",", ";")},{feature_selection_performance_metrics.ToString().Replace(",", ";")},{feature_selection_combinator.ToString().Replace(",", ";")},{a.experiment_id1},{a.experiment_id2},{a.experiment_id3},{a.iteration},{a.feature_set.alphabet},{a.feature_set.dimension},{a.feature_set.category},{a.feature_set.source},{a.feature_set.@group},{string.Join("/", a.feature_set.set_members.Select(c => c.member).Distinct().OrderBy(c => c).ToList())},{string.Join("/", a.feature_set.set_members.Select(c => c.perspective).Distinct().OrderBy(c => c).ToList())},{a.feature_set.set_id},{b.ToString()}").ToList()).ToList();

                f_lines_cm_all.Insert(0, feat_header);

                program.WriteAllLines(Path.Combine(this.results_output_folder, $"{start_date_time}_selected_features_CM_all_{iteration_loop}_{iteration_recalculate}_{iteration_select}.csv"), f_lines_cm_all);
            ...
            if (save_selected_features_list)
            {
                ...
                var feat_list = this.features_selected.SelectMany(a => a.set_members.OrderBy(c=>c.member).ThenBy(c=>c.perspective).Select(b => $"{feature_selection_type.ToString().Replace(",", ";")},{perf_selection_rule.ToString().Replace(",", ";")},{feature_selection_performance_metrics.ToString().Replace(",", ";")},{feature_selection_combinator.ToString().Replace(",", ";")},{a.source},{a.@group},{a.set_id},{b.fid},{b.alphabet},{b.dimension},{b.category},{b.source},{b.@group},{b.member},{b.perspective},{b.alphabet_id},{b.dimension_id},{b.category_id},{b.source_id},{b.@group_id},{b.member_id},{b.perspective_id}").ToList()).ToList();

                var f_lines = new List<string>();
                f_lines.Add(feat_header);
                f_lines.AddRange(feat_list);

                program.WriteAllLines(Path.Combine(this.results_output_folder, $"{start_date_time}_selected_features_{iteration_loop}_{iteration_recalculate}_{iteration_select}.csv"), f_lines);
            ...
            if (save_iteration_json)
            {
                ...
                {
                    program.WriteLineException(e, nameof(save_iteration_parameters), "", true, ConsoleColor.Red);
                }
        ...
        public run_svm_params run_svm_params;
        //public int rand_cv;
        //public int outer_cv;
        //public int inner_cv;
        ...
        public List<(int iteration, long milliseconds)> task_durations;
        ...
        public List<(List<(perf_selection_rules perf_rule, int rank)> perf_rule_ranks, string experiment_id1, string experiment_id2, string experiment_id3, char direction, feature_set feature_set, int iteration, score_metrics score_metrics, performance_measure.confusion_matrix cm_average, List<performance_measure.confusion_matrix> cm_all)> get_rank(rank_types rank_type)//, perf_selection_rules perf_selection_rule)
        {
            ...
            foreach (perf_selection_rules perf_select_rule1 in Enum.GetValues(typeof(perf_selection_rules)))
            {
                ...
                for (var _r = 0; _r < ranks2.Count; _r++)
                {
                    var r = _r;
                    var rk = ranks2[r];
        ...
        public void save_ranks()
        {
            ...
            var cm_average_csv_header = $@"iteration_loop,iteration_recalculate,iteration_select,baseline_recalculation_required,score_improved,forwards_score_improved,backwards_score_improved,go_forwards,can_go_forwards,go_backwards,can_go_backwards,direction_change,{p},CM,feature_selection_type,perf_selection_rule,feature_selection_performance_metrics,feature_selection_combinator,direction,iteration,is_selected,alphabet,category,dimension,source,group,members,perspectives,set_id,{score_metrics.csv_header},{performance_measure.confusion_matrix.csv_header}";
            ...
            foreach (rank_types rank_type in Enum.GetValues(typeof(rank_types)))
            {
                try
                {
                    ...
                    var rank_cm_average_csv = ranks2.Select((a, i) => $@"{iteration_loop},{iteration_recalculate},{iteration_select},{baseline_recalculation_required},{score_improved},{forwards_score_improved},{backwards_score_improved},{go_forwards},{can_go_forwards},{go_backwards},{can_go_backwards},{direction_change},{string.Join(",", a.perf_rule_ranks.Select(c => c.rank).ToList())},0,{feature_selection_type.ToString().Replace(",", ";")},{perf_selection_rule.ToString().Replace(",", ";")},{feature_selection_performance_metrics.ToString().Replace(",", ";")},{feature_selection_combinator.ToString().Replace(",", ";")},{a.direction},{a.iteration},{features_selected.Any(c => c.set_id == a.feature_set.set_id)},{a.feature_set.alphabet},{a.feature_set.category},{a.feature_set.dimension},{a.feature_set.source},{a.feature_set.@group},{string.Join("/", a.feature_set.set_members.Select(c => c.member).Distinct().OrderBy(c => c).ToList())},{string.Join("/", a.feature_set.set_members.Select(c => c.perspective).Distinct().OrderBy(c => c).ToList())},{a.feature_set.set_id},{a.score_metrics.ToString()},{a.cm_average.ToString()}").ToList();

                    rank_cm_average_csv.Insert(0, cm_average_csv_header);



                    program.WriteAllLines(Path.Combine(results_output_folder, $@"{start_date_time}_rank_cm_average_{rank_type.ToString()}_{iteration_loop}_{iteration_recalculate}_{iteration_select}.csv"), rank_cm_average_csv);




                    var rank_cm_all_csv = ranks2.SelectMany((a, i) => a.cm_all.Select((b, j) => $@"{iteration_loop},{iteration_recalculate},{iteration_select},{baseline_recalculation_required},{score_improved},{forwards_score_improved},{backwards_score_improved},{go_forwards},{can_go_forwards},{go_backwards},{can_go_backwards},{direction_change},{string.Join(", ", a.perf_rule_ranks.Select(c => c.rank).ToList())},{j.ToString()},{feature_selection_type.ToString().Replace(",", ";")},{perf_selection_rule.ToString().Replace(",", ";")},{feature_selection_performance_metrics.ToString().Replace(",", ";")},{feature_selection_combinator.ToString().Replace(",", ";")},{a.direction},{a.iteration},{features_selected.Any(c => c.set_id == a.feature_set.set_id)},{a.feature_set.alphabet},{a.feature_set.category},{a.feature_set.dimension},{a.feature_set.source},{a.feature_set.@group},{string.Join("/", a.feature_set.set_members.Select(c => c.member).Distinct().OrderBy(c => c).ToList())},{string.Join("/", a.feature_set.set_members.Select(c => c.perspective).Distinct().OrderBy(c => c).ToList())},{a.feature_set.set_id},{a.score_metrics.ToString()},{b.ToString()}").ToList()).ToList();

                    rank_cm_all_csv.Insert(0, cm_average_csv_header);

                    program.WriteAllLines(Path.Combine(results_output_folder, $@"{start_date_time}_rank_cm_all_{rank_type.ToString()}_{iteration_loop}_{iteration_recalculate}_{iteration_select}.csv"), rank_cm_all_csv);



                    if (program.write_console_log) program.WriteLine($"{iteration_loop} :::{feature_selection_type}: {rank_type.ToString()} iteration ranks:");

                    for (var _index = 0; _index < ranks2.Count; _index++)
                    {
                        var index = _index;
                        var a = ranks2[index];

                        if (program.write_console_log) program.WriteLine($"{iteration_loop} :{rank_type.ToString()}: {index} -> {a.feature_set.set_id}.{a.feature_set.source}.{a.feature_set.@group} -> {a.score_metrics.ToText()}");
                    }

                }
                catch (Exception e)
                {
                    program.WriteLineException(e, nameof(save_ranks));
                }
            }
        ...
        //    lock (text_log_lock)
        //    {
        //        if (program.write_console_log) program.WriteLine($@"iteration: {iteration}.  {text}");
        ...
        public void set_defaults()
        {
            ...
            //this.if (program.write_console_log) program.WriteLine(-1, "Set default parameters");


            this.dataset_instance_list = null;

            this.run_svm_params = new run_svm_params();
            //rand_cv = 1;
            //outer_cv = 5;
            //inner_cv = 5;

            this.task_durations = new List<(int iteration, long milliseconds)>();
        ...
        public static string serialise_json(feature_selection_unidirectional feature_selection_unidirectional)
        {
            ...
            catch (Exception e)
            {
                ...
                program.WriteLineException(e, nameof(serialise_json), "", true, ConsoleColor.Yellow);
        ...
        public static feature_selection_unidirectional deserialise_json(string serialized_json)
        {
            ...
            catch (Exception e)
            {
                ...
                program.WriteLineException(e, nameof(deserialise_json),"", true, ConsoleColor.Yellow);
        ...
        public feature_selection_unidirectional(
            ...
            performance_measure.confusion_matrix.cross_validation_metrics feature_selection_performance_metrics = performance_measure.confusion_matrix.cross_validation_metrics.F1S, //.MCC | performance_measure.confusion_matrix.cross_validation_metrics.API_All,
            ...
            )
        {
            ...
            //if (program.write_console_log) program.WriteLine(-1, "Setting algorithm parameters:");
       ...
       new string[]
        ...
        public cross_validation.run_svm_return classify(List<feature_set> feature_set_list, string experiment_id1, string experiment_id2, string experiment_id3)
        {
            ...
            var classify_run_svm_params = new run_svm_params(this.run_svm_params)
            {
            ...
            var svm_return = cross_validation.run_svm(classify_run_svm_params, cts.Token, inner_cv_svm_implementation, outer_cv_svm_implementation);
        ...
        public (List<feature_set> selected_features, score_metrics performance) run_unidirectional_feature_selection()
        {
            ...
            var max_forwards_bad_iterations = 2;
            var forwards_bad_iteration_consecutive = 0;

            var max_backwards_bad_iterations = 2;
            var backwards_bad_iteration_consecutive = 0;
            ...
            do
            {
                ...
                if (program.write_console_log) program.WriteLine($@"------------------------------------------------------------------------------------------------------------------");
                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Starting: Iteration #: {iteration_loop} with ranking score: {baseline_score}.  Algorithm: {feature_selection_type}.");
                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Starting: Selected features at start; {features_selected.Count} feature groups; {features_selected.Sum(a => a.set_members.Count)} feature members;");
                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Starting: Selected features groups at start; [{string.Join(", ", features_selected.Select(a => $"{a.source}.{a.@group}").ToList())}]");
                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Starting: Selected features groups and members at start; [{string.Join(", ", features_selected.SelectMany(a => a.set_members.Select(b => $"{b.dimension}.{b.alphabet}.{b.category}.{b.source}.{b.@group}.{b.member}.{b.perspective}").ToList()).ToList())}]");
                ...
                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Search directions: can_go_forwards={can_go_forwards}; go_forwards={go_forwards}; can_go_backward={can_go_backwards}; go_backwards={go_backwards}; ");
                ...
                if (baseline_recalculation_required)
                {
                    ...
                    if (program.write_console_log) program.WriteLine($@"{iteration_loop} Baseline ranking: Recalculating baseline rank:");
                    if (program.write_console_log) program.WriteLine($@"{iteration_loop} Baseline ranking: features: groups: {features_selected_total_group} features: {features_selected_total_features}. List: {features_selected_str}.");
                    iteration_recalculate++;
                    ...
                    var result = new iterative_task()
                    {
                        ...
                        task_id = -1,
                        duration_method = sw_method.ElapsedMilliseconds,
                        duration_iteration = sw_iteration.ElapsedMilliseconds,
                        duration_index = 0,
                        duration_task = 0,
                    };
                    ...
                    if (program.write_console_log) program.WriteLine($@"{iteration_loop} Baseline ranking: New baseline rank... Previous rank: {baseline_score.score_before}.  New rank: {baseline_score.score_after}.  Change: {baseline_score.score_change}.  Improved: {score_improved}.");

                    if (program.write_console_log) program.WriteLine($@"{iteration_loop} Baseline ranking: Selection Classes Average Performance: {string.Join("; ", svm_cms_perf_classes_average?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");
                    svm_cms_all.ForEach(x =>
                    {
                        if (program.write_console_log) program.WriteLine($@"{iteration_loop} Baseline ranking: All Class Performance: {string.Join("; ", x?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");
                    });


                    if (!score_improved)
                    {
                        if (go_forwards && can_go_forwards && forwards_bad_iteration_consecutive < max_forwards_bad_iterations)
                        {
                            score_improved = true;

                            forwards_bad_iteration_consecutive++;
                        }

                        if (go_backwards && can_go_backwards && backwards_bad_iteration_consecutive < max_backwards_bad_iterations)
                        {
                            score_improved = true;

                            backwards_bad_iteration_consecutive++;
                        }
                    }
                ...
                else //if (!baseline_recalculation_required)
                {
                    ...
                    if ((go_forwards && can_go_forwards && !forwards_max_features_reached) || (go_backwards && can_go_backwards && !backwards_min_features_reached))
                    {
                        ...
                        var for_loop_tasks = new List<Task<iterative_task>>();
                        ...
                        for (var _features_input_index = 0; _features_input_index < features_input.Count; _features_input_index++)
                        {
                            var features_input_index = _features_input_index;
                        ...
                        for (var _features_input_index = 0; _features_input_index < features_input.Count; _features_input_index++)
                        {
                            ...
                            if (program.write_console_log) program.WriteLine($@"{iteration_loop} {features_input_index} Loop: features_input_index={(features_input_index + 1)}/{features_input.Count}");

                            var iteration_feature_set = features_input[features_input_index];

                            if (iteration_feature_set == null || iteration_feature_set.set_members == null || iteration_feature_set.set_members.Count == 0) continue;// return null;//continue;
                            ...
                            if (feature_already_selected) // if feature already selected, go backwards if requested/possible
                            {
                                if (go_backwards && can_go_backwards && !backwards_min_features_reached)
                                {
                                    ...
                                    if (backwards_good_feature || forwards_good_feature)
                                    {
                                        ...
                                        if (program.write_console_log) program.WriteLine($@"{iteration_loop} {features_input_index} Loop: Backwards skipping feature set: [{iteration_feature_set.source}.{iteration_feature_set.@group}]");

                                        if (features_input_index != final_backwards_iteration_index) continue; //return null;//continue;
                                    }
                                    else
                                    {
                                    ...
                                    if ((features_input_index == final_backwards_iteration_index && features_backwards_buffer.Count > 0) || (features_selected.Count - features_backwards_buffer.Count <= backwards_min_features && features_backwards_buffer.Count > 0) || features_backwards_buffer.Count >= backwards_max_features_to_combine_per_iteration)
                                    {
                                        ...
                                        var backwards_task = Task.Run(() =>
                                        {
                                            ...
                                            {
                                                if (program.write_console_log) program.WriteLine($@"{internal_b_iteration_loop} {internal_b_loop_index} Task: Error: svm return is empty");
                                                return null;
                                            }
                                            else
                                            {
                                                ...
                                                var result = new iterative_task()
                                                {
                                                    ...
                                                    duration_method = internal_b_sw_method.ElapsedMilliseconds,
                                                    duration_iteration = internal_b_sw_iteration.ElapsedMilliseconds,
                                                    duration_index = internal_b_sw_index.ElapsedMilliseconds,
                                                    duration_task = task_sw_task.ElapsedMilliseconds,
                                                };
                                                ...
                                                if (program.write_console_log) program.WriteLine($@"{internal_b_iteration_loop} {internal_b_loop_index} Task: Backwards ranking: Task: {string.Join("; ", result.AsArray().Select(a => $"{a.name}={a.value};").ToList())}");
                                                if (program.write_console_log) program.WriteLine($@"{internal_b_iteration_loop} {internal_b_loop_index} Task: Backwards ranking: Groups: {internal_b_features_backwards_buffer.Count}; Features: {internal_b_features_backwards_buffer.Sum(a => a.set_members.Count)}; [{string.Join("; ", internal_b_features_backwards_buffer.Select(c => $"{c.source}.{c.@group}").ToList())}] -> {feature_selection_metrics_average}");
                                                if (program.write_console_log) program.WriteLine($@"{internal_b_iteration_loop} {internal_b_loop_index} Task: Forwards ranking: Selection Classes Average Performance: {string.Join("; ", svm_cms_perf_classes_average?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");


                                                svm_cms_all.ForEach(x =>
                                                {
                                                    if (program.write_console_log) program.WriteLine($@"{internal_b_iteration_loop} {internal_b_loop_index} Task: Forwards ranking: All Class Performance: {string.Join("; ", x?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");
                                                });
                                                ...
                                                lock (tasks_lock)
                                                {
                                                    var dur_this_iteration = for_loop_tasks.Where(a => a.IsCompleted).Select(a => a.Result.duration_task).ToList();

                                                    if (dur_this_iteration.Count > 0)
                                                    {
                                                        var eta_min = TimeSpan.FromMilliseconds((long)dur_this_iteration.Min() * (long)features_input.Count);
                                                        var eta_av = TimeSpan.FromMilliseconds((long)dur_this_iteration.Average() * (long)features_input.Count);
                                                        var eta_max = TimeSpan.FromMilliseconds((long)dur_this_iteration.Max() * (long)features_input.Count);

                                                        if (program.write_console_log) program.WriteLine($"{internal_b_iteration_loop} {internal_b_loop_index} Projected time: iteration using current: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                    }
                                                }

                                                var dur_last_iteration = task_durations.Where(a => a.iteration == internal_b_iteration_loop - 1).Select(a => a.milliseconds).ToList();
                                                if (dur_last_iteration.Count > 0)
                                                {
                                                    var eta_min = TimeSpan.FromMilliseconds((long)dur_last_iteration.Min() * (long)features_input.Count);
                                                    var eta_av = TimeSpan.FromMilliseconds((long)dur_last_iteration.Average() * (long)features_input.Count);
                                                    var eta_max = TimeSpan.FromMilliseconds((long)dur_last_iteration.Max() * (long)features_input.Count);

                                                    if (program.write_console_log) program.WriteLine($"{internal_b_iteration_loop} {internal_b_loop_index} Projected time: iteration using last: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                }

                                                var dur_all_iteration = task_durations.Select(a => a.milliseconds).ToList();
                                                if (dur_all_iteration.Count > 0)
                                                {
                                                    var eta_min = TimeSpan.FromMilliseconds((long)dur_all_iteration.Min() * (long)features_input.Count);
                                                    var eta_av = TimeSpan.FromMilliseconds((long)dur_all_iteration.Average() * (long)features_input.Count);
                                                    var eta_max = TimeSpan.FromMilliseconds((long)dur_all_iteration.Max() * (long)features_input.Count);

                                                    if (program.write_console_log) program.WriteLine($"{internal_b_iteration_loop} {internal_b_loop_index} Projected time: iteration using all: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                }
                                        ...
                                        lock (tasks_lock)
                                        {
                                            for_loop_tasks.Add(backwards_task);
                                        }
                            ...
                            else // if feature *not* already selected, go forwards if requested/possible
                            {
                                if (go_forwards && can_go_forwards && !forwards_max_features_reached)
                                {
                                    if (backwards_bad_feature || forwards_bad_feature)
                                    {
                                        ...
                                        if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Forwards skipping feature set: [{iteration_feature_set.source}.{iteration_feature_set.@group}]");


                                        if (features_input_index != final_forwards_iteration_index) continue;// return null;//continue;
                                    }
                                    else
                                    {
                                    ...
                                    if ((features_input_index == final_forwards_iteration_index && features_forwards_buffer.Count > 0) || (features_selected.Count + features_forwards_buffer.Count >= forwards_max_features && features_forwards_buffer.Count > 0) || features_forwards_buffer.Count >= forwards_max_features_to_combine_per_iteration)
                                    {
                                        ...
                                        var forwards_task = Task.Run(() =>
                                        {
                                            ...
                                            if (svm_return == null || svm_return.run_svm_return_data == null || svm_return.run_svm_return_data.Count == 0)
                                            {
                                                if (program.write_console_log) program.WriteLine($"{internal_f_iteration_loop} Loop: Error: svm return is empty");
                                                return null;
                                            }
                                            else
                                            {
                                                ...
                                                var result = new iterative_task()
                                                {
                                                    ...
                                                    duration_method = internal_f_sw_method.ElapsedMilliseconds,
                                                    duration_iteration = internal_f_sw_iteration.ElapsedMilliseconds,
                                                    duration_index = internal_f_sw_index.ElapsedMilliseconds,
                                                    duration_task = task_sw_task.ElapsedMilliseconds,
                                                };
                                                ...
                                                if (program.write_console_log) program.WriteLine($@"{internal_f_iteration_loop} {internal_f_loop_index} Task: Forwards ranking: Task: {string.Join("; ", result.AsArray().Select(a => $"{a.name}={a.value};").ToList())}");
                                                if (program.write_console_log) program.WriteLine($@"{internal_f_iteration_loop} {internal_f_loop_index} Task: Forwards ranking: Groups: {internal_f_features_forwards_buffer.Count}; Features: {internal_f_features_forwards_buffer.Sum(a => a.set_members.Count)}; [{string.Join("; ", internal_f_features_forwards_buffer.Select(c => $"{c.source}.{c.@group}").ToList())}] -> {feature_selection_metrics_average}");
                                                if (program.write_console_log) program.WriteLine($@"{internal_f_iteration_loop} {internal_f_loop_index} Task: Forwards ranking: Selection Classes Average Performance: {string.Join("; ", svm_cms_perf_classes_average?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");
                                                svm_cms_all.ForEach(x =>
                                                {
                                                    if (program.write_console_log) program.WriteLine($@"{internal_f_iteration_loop} {internal_f_loop_index} Task: Forwards ranking: All Class Performance: {string.Join("; ", x?.get_value_strings().Select(a => $"{a.name}={a.value};").ToList())}");
                                                });
                                                ...
                                                lock (tasks_lock)
                                                {
                                                    ...
                                                    if (dur_this_iteration.Count > 0)
                                                    {
                                                        ...
                                                        if (program.write_console_log) program.WriteLine($"{internal_f_iteration_loop} {internal_f_loop_index} Projected time: iteration using current: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                    }
                                                }

                                                var dur_last_iteration = task_durations.Where(a => a.iteration == internal_f_iteration_loop - 1).Select(a => a.milliseconds).ToList();
                                                if (dur_last_iteration.Count > 0)
                                                {
                                                    var eta_min = TimeSpan.FromMilliseconds((long)dur_last_iteration.Min() * (long)features_input.Count);
                                                    var eta_av = TimeSpan.FromMilliseconds((long)dur_last_iteration.Average() * (long)features_input.Count);
                                                    ...
                                                    if (program.write_console_log) program.WriteLine($"{internal_f_iteration_loop} {internal_f_loop_index} Projected time: iteration using last: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                }
                                                ...
                                                if (dur_all_iteration.Count > 0)
                                                {
                                                    ...
                                                    if (program.write_console_log) program.WriteLine($"{internal_f_iteration_loop} {internal_f_loop_index} Projected time: iteration using all: Min: {eta_min.Days}:{eta_min.Hours}:{eta_min.Minutes}:{eta_min.Seconds}, Average: {eta_av.Days}:{eta_av.Hours}:{eta_av.Minutes}:{eta_av.Seconds}, Max: {eta_max.Days}:{eta_max.Hours}:{eta_max.Minutes}:{eta_max.Seconds}", true, ConsoleColor.Cyan);
                                                }
                                                */
                                        ...
                                        lock (tasks_lock)
                                        {
                                            for_loop_tasks.Add(forwards_task);
                                        }
                            ...
                            //lock (tasks_lock)
                            //{
                            //    for_loop_tasks.Add(for_loop_task);
                            //}

                            var max_tasks = 2;

                            if (run_svm_params.kernel_parameter_search_methods.Count == 1 && run_svm_params.kernel_parameter_search_methods.First() == libsvm_caller.kernel_parameter_search_method.none)
                            {
                                max_tasks = Environment.ProcessorCount * 2;
                            }

                            if (run_svm_params.run_remote)
                            {
                                var load_hosts = program.load_hosts();

                                if (load_hosts != null && load_hosts.Count > 0)
                                {
                                    max_tasks = load_hosts.Count * (cross_validation_remote.max_tcp_clients > 0 ? cross_validation_remote.max_tcp_clients : 2); // * 2;
                                }
                            }

                            //else
                            //{
                            //    max_tasks = Environment.ProcessorCount * 20;
                            //}

                            if (max_tasks > 0 && for_loop_tasks != null && for_loop_tasks.Count > 0)
                            {
                                var incomplete_for_loop_tasks = for_loop_tasks.Where(a => !a.IsCompleted).ToList();

                                while (incomplete_for_loop_tasks != null && incomplete_for_loop_tasks.Count > 0 && incomplete_for_loop_tasks.Count >= max_tasks)
                                {
                                    if (program.write_console_log) program.WriteLine($@"{iteration_loop} {features_input_index} Loop: {nameof(run_unidirectional_feature_selection)}(): Task.WaitAny(incomplete_tasks.ToArray<Task>());", true, ConsoleColor.Cyan);
                                    try
                                    {
                                        Task.WaitAny(incomplete_for_loop_tasks.ToArray<Task>());
                                    }
                                    catch (Exception e)
                                    {
                                        program.WriteLineException(e, nameof(run_unidirectional_feature_selection), $@"{iteration_loop} {features_input_index} Loop", true, ConsoleColor.Cyan);
                                    }

                                    incomplete_for_loop_tasks = for_loop_tasks.Where(a => !a.IsCompleted).ToList();
                                }
                            }

                            //var min_free_cpu = (double)1 / (double)Environment.ProcessorCount;
                            //var min_free_ram = 512;

                            //var free_cpu = system_perf.get_average_cpu_free(10);
                            //var free_ram = system_perf.get_average_ram_free_mb(5);
                        ...
                        try
                        {
                            Task.WaitAll(for_loop_tasks.ToArray<Task>());
                        }
                        catch (Exception e)
                        {
                            program.WriteLineException(e, nameof(run_unidirectional_feature_selection));
                        }

                        for_loop_tasks = for_loop_tasks.Where(a => a.Result != null).ToList();
                        var backwards_tasks = for_loop_tasks.Where(a => a.Result.direction == 'B').ToList();
                        var forwards_tasks = for_loop_tasks.Where(a => a.Result.direction == 'F').ToList();

                        if (go_backwards && can_go_backwards && !backwards_min_features_reached)
                        {
                            if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: {nameof(run_unidirectional_feature_selection)}(): Task.WaitAll(backwards_tasks.ToArray<Task>());", true, ConsoleColor.Cyan);
                            //Task.WaitAll(backwards_tasks.ToArray<Task>());
                            backwards_tasks = backwards_tasks.OrderBy(a => a.Result.loop_index).ToList();
                            ...
                            backwards_selection_feature_rankings_bad_features = backwards_selection_feature_rankings.Where(a => a.baseline_score.score_after >= (baseline_score.score_after))/*.OrderByDescending(a => a.baseline_score_after)*/.ToList();

                            if (this.feature_selection_type == feature_selection_types.backwards)
                            {
                                if (backwards_selection_feature_rankings_bad_features == null || backwards_selection_feature_rankings_bad_features.Count == 0)
                                {
                                    if (max_backwards_bad_iterations > 0 && backwards_bad_iteration_consecutive < max_backwards_bad_iterations)
                                    {
                                        backwards_bad_iteration_consecutive++;

                                        backwards_selection_feature_rankings_bad_features = backwards_selection_feature_rankings.OrderByDescending(a => a.baseline_score.score_after).Take(backwards_max_features_to_remove_per_iteration).ToList();
                                    }
                                }
                                else
                                {
                                    backwards_bad_iteration_consecutive = 0;
                                }
                            }
                            ...
                            if (backwards_selection_feature_rankings_taken_for_removal != null && backwards_selection_feature_rankings_taken_for_removal.Count > 0)
                            {
                                ...
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Backwards removed features: {backwards_features_taken_names.Count} -> [{string.Join(", ", backwards_features_taken_names.Select(b => $"{b.source}.{b.@group}").ToList())}]");
                            ...
                            {
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Backwards set: {backwards_selection_feature_rankings.Count} -> [{string.Join(", ", backwards_selection_feature_rankings.SelectMany(c => c.feature_set_list.Select(b => $"{b.source}.{b.@group}").ToList()).ToList())}]");
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Backwards set taken: {backwards_selection_feature_rankings_taken_for_removal.Count} -> [{string.Join(", ", backwards_selection_feature_rankings_taken_for_removal.SelectMany(c => c.feature_set_list.Select(b => $"{b.source}.{b.@group}").ToList()).ToList())}]");
                            }
                        }
                        else
                        {
                        ...
                        if (go_forwards && can_go_forwards && !forwards_max_features_reached)
                        {
                            if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: {nameof(run_unidirectional_feature_selection)}(): Task.WaitAll(forwards_tasks.ToArray<Task>());", true, ConsoleColor.Cyan);
                            //Task.WaitAll(forwards_tasks.ToArray<Task>());
                            forwards_tasks = forwards_tasks.OrderBy(a => a.Result.loop_index).ToList();
                            ...
                            forwards_selection_feature_rankings_good_features = forwards_selection_feature_rankings.Where(a => a.baseline_score.score_after > random_baseline && a.baseline_score.score_after > (baseline_score.score_after)) /*.OrderByDescending(a => a.baseline_score_after)*/.ToList();

                            if (this.feature_selection_type == feature_selection_types.forwards)
                            {
                                if (forwards_selection_feature_rankings_good_features == null || forwards_selection_feature_rankings_good_features.Count == 0)
                                {
                                    if (max_forwards_bad_iterations > 0 && forwards_bad_iteration_consecutive < max_forwards_bad_iterations)
                                    {
                                        forwards_bad_iteration_consecutive++;

                                        forwards_selection_feature_rankings_good_features = forwards_selection_feature_rankings.OrderByDescending(a => a.baseline_score.score_after).Take(forwards_max_features_to_add_per_iteration).ToList();
                                    }
                                }
                                else
                                {
                                    forwards_bad_iteration_consecutive = 0;
                                }
                            }
                            ...
                            if (forwards_selection_feature_rankings_taken_for_insertion != null && forwards_selection_feature_rankings_taken_for_insertion.Count > 0)
                            {
                                ...
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Forwards added features: {forwards_features_taken_feature_sets.Count} -> [{string.Join(", ", forwards_features_taken_feature_sets.Select(a => a.source + "." + a.@group).ToList())}]");
                            ...
                            {
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Forwards set: {forwards_selection_feature_rankings.Count} -> [{string.Join(", ", forwards_selection_feature_rankings)}]");
                                if (program.write_console_log) program.WriteLine($@"{iteration_loop} Loop: Forwards set taken: {forwards_selection_feature_rankings_taken_for_insertion.Count} -> [{string.Join(", ", forwards_selection_feature_rankings_taken_for_insertion.SelectMany(a => a.feature_set_list.Select(b => b.source + "." + b.@group).ToList()).ToList())}]");
                            }
                        }
                        else
                        {
                        ...
                        if ((!score_improved || backwards_min_features_reached) && !backwards_finished && go_backwards && (feature_selection_type == feature_selection_types.backwards_then_forwards || feature_selection_type == feature_selection_types.backwards_then_forwards_repeated_until_convergence))
                        {
            ...
            } while (baseline_recalculation_required || 
                     
                     (
                         (iteration_select < iteration_select_max || iteration_select_max < 0) && 
                         (
                             (score_improved && (can_go_backwards || can_go_forwards)) || 
                             ((go_backwards && direction_change && can_go_forwards) || (go_forwards && direction_change && can_go_backwards)))
                         )
